...
export const calculateColor = ({ container, targetRef, offset }) => {
...
export const calculateHueColor = ({ container, targetRef, offset }) => {
	const { width, height } = container.value.getBoundingClientRect();
	const hue = Math.floor(360 * (offset.x / width));
	return hue;
};

export const calculateOpacityColor = ({ container, targetRef, offset }) => {
	const { width, height } = container.value.getBoundingClientRect();
...
export const colorToPoint = ({ container, s, v }) => {
	const { width, height } = container.value.getBoundingClientRect();
...
export const hueToPoint = ({ container, h }) => {
	const { width, height } = container.value.getBoundingClientRect();
	const hueX = Math.floor((h / 360) * width);
	return hueX;
};

export const opacityToPoint = ({ container, s }) => {
	const { width, height } = container.value.getBoundingClientRect();
	const opaX = s * width;
	return opaX;
};
